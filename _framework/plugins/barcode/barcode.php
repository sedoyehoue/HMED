...
class Bar{
	...
	static function code($barcode='dagrge',$width='160',$height='80',$text='1',$format='PNG',$quality='100',$type='1'){
		ini_set('display_errors', 0);
		switch ($type){
        	default:
				$type = 1;
        	case 1:
				self::Barcode39($barcode, $width, $height, $quality, $format, $text);
			break;          
		}
	}
	...
	//-----------------------------------------------------------------------------
	static function Barcode39($barcode, $width, $height, $quality, $format, $text)
	{
			switch ($format)
			{
					default:
		...
		or die ('Cannot Initialize new GD image stream');
			...
			if (($NarrowBar == 0) || ($NarrowBar == $WideBar) || ($NarrowBar == $QuietBar) || ($WideBar == 0) || ($WideBar == $QuietBar) || ($QuietBar == 0))
			{
					...
					self::OutputImage ($im, $format, $quality);
					exit;
			}
	...
	//-----------------------------------------------------------------------------
	static function OutputImage($im, $format, $quality)
	{
			switch ($format)
			{
					case 'JPEG': 
